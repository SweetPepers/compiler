    ...
    if (equal(Tok, "##")) {
      ...
      if (Arg) {
        if (Arg->Tok->Kind != TK_EOF) {
          // 拼接当前终结符和（##右边）实参
          *Cur = *paste(Cur, Arg->Tok); // 只拼接了右边实参的第一个终结符然后重新token
...
static bool expandMacro(Token **Rest, Token *Tok) {
  ...
  // 我们取宏终结符和右括号的交集，并将其用作新的隐藏集。// todo
